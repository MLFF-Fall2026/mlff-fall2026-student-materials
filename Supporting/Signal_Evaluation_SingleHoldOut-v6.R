# Purpose: Evaluate the efficacy of a trading signal over a single hold out period

signal_evaluation_singleholdout <- function(
  test_returns,
  Meta,
  return_threshold,
  signal_position
  ...
  summarise_stats <- function(df, group_vars, value_col = "test_returns") {
    ...
    df %>%
      ...
      summarise(
        ...
        skew = ifelse(
          n > 2,
          e1071::skewness(!!value_sym, na.rm = TRUE),
          NA_real_
        ),
        kurt = ifelse(
          n > 3,
          e1071::kurtosis(!!value_sym, na.rm = TRUE),
          NA_real_
        ),
      ...
      mutate(
        group = if (length(group_vars) == 1) {
          ...
          apply(
            select(cur_data(), all_of(group_vars)),
            1,
            paste,
            collapse = " | "
          )
  ...
  calculate_ic <- function(signal_position, ret) {
    ...
    ic <- if (sigma_s > 0 && sigma_r > 0) {
      cov_sr / (sigma_s * sigma_r)
    } else {
      NA_real_
    }
  ...
  calculate_hit_rate <- function(
    ...
    list(
      hit_rate_overall = p_hat,
      overall_hits = hits_overall,
      overall_n = n,
      z_overall = z,
      p_overall = p_val,
      hit_rate_acted = p_hat_a,
      acted_hits = hits_acted,
      acted_n = n_acted,
      z_acted = z_a,
      p_acted = p_a,
      coverage = n_acted / n
  ...
  eval_confusionmatrix <- function(
    df,
    pos_outcome_value,
    prediction_signal_value
  ) {
  ...
  if ("return_category" %in% names(TradeResults)) {
    names(TradeResults)[
      names(TradeResults) == "return_category"
    ] <- "Test Period Returns"
  }
  ...
  ic_all <- calculate_ic(
    evaluation_data$signal_position,
    evaluation_data$test_returns
  )
  ic_act <- calculate_ic_acted_only(
    evaluation_data$signal_position,
    evaluation_data$test_returns
  )
  ...
  # -----------------------
  long_case <- eval_confusionmatrix(
    evaluation_data,
    pos_outcome_value = 1L,
    prediction_signal_value = 1L
  )
  short_case <- eval_confusionmatrix(
    evaluation_data,
    pos_outcome_value = -1L,
    prediction_signal_value = -1L
  )
  neutral_case <- eval_confusionmatrix(
    evaluation_data,
    pos_outcome_value = 0L,
    prediction_signal_value = 0L
  )
  ...
  # -----------------------
  compute_return_comparison_table <- function(
    df,
    ret_col = "test_returns",
    sig_col = "signal_position"
  ) {
  ...
  p_signal <- ggplot(evaluation_data, aes(x = signal, y = test_returns)) +
    ...
    labs(
      title = "Return Distribution by Signal",
      y = "Test returns",
      x = NULL
    ) +
    theme_minimal()
  ...
  p_facet <- ggplot(evaluation_data, aes(x = signal, y = test_returns)) +
    ...
    labs(
      title = "Signal performance conditional on return bucket",
      y = "Test returns",
      x = NULL
    ) +
    theme_minimal()
  ...
  p_combo <- ggplot(evaluation_data, aes(x = combo, y = test_returns)) +
    ...
    labs(
      title = "Return distributions for each Signal × Outcome intersection",
      x = NULL,
      y = "Test returns"
    ) +
    theme_minimal() +
