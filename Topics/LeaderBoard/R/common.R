...
read_config <- function(path = "Topics/LeaderBoard/config/teams.yml") {
  ...
  if (cfg$trading$start_date > cfg$trading$end_date) {
    stop("Production start_date must be on or before end_date.")
  }
  if (cfg$prototype$start_date > cfg$prototype$end_date) {
    stop("Prototype start_date must be on or before end_date.")
  }
